(function() {
	'use strict';
	var controllerId = 'catProvinceController';
	
	angular.module('MetronicApp').controller(controllerId, catProvinceController);
	
	function catProvinceController($scope,$templateCache, $rootScope, $timeout, gettextCatalog, 
			kendoConfig, $kWindow,catProvinceService,
...
//				status:1
				...
				dataSource:{
					...
					transport: {
						read: {
							...
							url: Constant.BASE_SERVICE_URL + RestEndpoint.CAT_PROVINCE_SERVICE_URL+ "/doSearch",
		...
		vm.exportFile = function exportFile() {
			...
			var data = catProvinceService.doSearch(vm.catProvinceSearch);
			console.log(data);
			catProvinceService.doSearch(vm.catProvinceSearch).then(function(d){
				CommonService.exportFile(vm.catProvinceGrid,d.data,vm.listRemove,vm.listConvert,"Danh sách tỉnh thành");
			});
