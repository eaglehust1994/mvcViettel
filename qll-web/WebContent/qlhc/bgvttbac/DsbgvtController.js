				...
				var dataSource = new kendo.data.DataSource({
					...
					schema: {
	                    model: {
	                        id: "vttbGrid",
	                    	fields: {
	                    		handoverQuantity: { validation: { /*required: true , */required: { message: "Không bỏ trống đối tượng kiểm tra" } } },
	                    		actualReceiveQuantity: { validation: { /*required: true , */required: { message: "Không bỏ trống đối tượng kiểm tra" } } }
	                        }
	                    }
	                }
				});
			...
			function boxung() {
				if(vm.showDetail == true) {
					...
					bgvttbacService.getwareExpCmdByConstrt(vm.constrObj).then(function(result) {
	                    ...
	                    var title = gettextCatalog.getString("Danh sách phiếu đã xuất kho ra công trình");
