(function() {
	...
	var controllerId = 'ImportTitAziController';

	angular.module('MetronicApp').controller(controllerId,
			ImportTitAziController);

	/* @ngInject */
	function ImportTitAziController(
			$scope, $rootScope, $timeout, Constant,
			kendoConfig, ProposalEvaluation,titAziConstrAcceptService, gettextCatalog, $kWindow,
		...
		vm.downloadTempleKLXLHT = function (){
			titAziConstrAcceptService.downloadTempleKLXLHT(vm.item).then(function () {
			...
			if (input.files.length) {
				...
				reader.onload = function (e) {
					...
					for (var i = 0; i < worklistexcell.length; i++) {
						if(worklistexcell[i].azimuthDirectAAdjust){
							check=true;
						}
					}
					...
					$rootScope.$broadcast("importKLXL", worklistexcell);
					$("div.k-window-actions > a.k-window-action > span.k-i-close").click();
				};
			} else {
