(function() {
	'use strict';
	var controllerId = 'workItemQuotaMoneyController';
	
	angular.module('MetronicApp').controller(controllerId, workItemQuotaMoneyController);
	
	function workItemQuotaMoneyController($scope,$templateCache, $rootScope, $timeout, gettextCatalog, 
...
//		
		...
		function refreshGrid(d) {
		  ...
		  function add(){
			vm.workItemQuota={};
			 var teamplateUrl="ims/workItemQuotaMoney/workItemQuotaMoneyPopup.html";
			 var title="Thêm mới định mức dòng tiền";
			 var windowId="APP_PARAM";
			 htmlCommonService.populatePopupCreate(teamplateUrl,title,null,vm,windowId,true,'800','300',"workItemQuotaId"); 
		 }
		...
		function edit(dataItem){			
			...
			var teamplateUrl="ims/workItemQuotaMoney/workItemQuotaMoneyPopup.html";
			var title="Cập nhật định mức dòng tiền";
			var windowId="APP_PARAM";
			$("#workItemQuotaGrid").data('kendoGrid').dataSource.read();
			$("#workItemQuotaGrid").data('kendoGrid').refresh();
			htmlCommonService.populatePopupCreate(teamplateUrl,title,vm.workItemQuota,vm,windowId,false,'800','300',"workItemQuotaId");
		}
		
                vm.import=function(){
                    vm.workItemQuota={};
			 var teamplateUrl="ims/workItemQuotaMoney/importWorkItemQuota.html";
			 var title="Import định mức dòng tiền";
			 var windowId="APP_PARAM";
			 htmlCommonService.populatePopupCreate(teamplateUrl,title,null,vm,windowId,true,'800','200'); 
                }
		
		vm.save= function(data,isCreateNew){
			vm.workItemQuota.quotaType = 2;
...
// }
		...
		vm.gridColumnShowHideFilter = function (item) { 
            ...
            vm.getExcelTemplate = function(){
    			var fileName="FileBieuMau_DMHangMucDongTien";
...
//     						 $('#workItemQuotaId').val(null);
    	     ...
    	     vm.patternWorkItemTypeOptions={
      	            ...
      	            dataSource: {
      	                ...
      	                transport: {
      	                    read: function(options) {
      	                        return Restangular.all("catWorkItemTypeServiceRest/getForAutoComplete").post({pageSize:10, page:1, keySearch:$("#"+proccessFormId("workItemType")).val().trim()}).then(function(response){
      	                            options.success(response);
...
//    			alert("open"+vm.isLive);
	        ...
	        function submit(data){
				...
				if($("#filePackage")[0].files[0].name.split('.').pop() !=='xls' && $("#filePackage")[0].files[0].name.split('.').pop() !=='xlsx' ){
			     ...
			     return   $.ajax({
		            url: Constant.BASE_SERVICE_URL+RestEndpoint.WORK_ITEM_QUOTA_SERVICE_URL+"/importWorkItemQuota?folder="+ vm.folder+"&quotaType=2",
		            type: "POST",
		            data: formData,
