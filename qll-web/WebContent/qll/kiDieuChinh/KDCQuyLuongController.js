...
(function() {
	'use strict';
	var controllerId = 'KDCQuyLuongController';
	
	angular.module('MetronicApp').controller(controllerId, KDCQuyLuongController);
	
	function KDCQuyLuongController($scope, $rootScope, $timeout, gettextCatalog, 
			kendoConfig, $kWindow,
			CommonService, PopupConst, Restangular, RestEndpoint,Constant, KDCQuyLuongService,
			
			) {
		var vm = this;
		vm.showSearch = true;
		vm.validatorOptions = kendoConfig.get('validatorOptions');
		vm.KDCQuyLuongSearch = {};
				...
				toolbar: [
		                    {
			                    ...
			                    '<label ng-repeat="column in vm.KDCQuyLuongGrid.columns.slice(1,vm.KDCQuyLuongGrid.columns.length)| filter: vm.gridColumnShowHideFilter">'+
				...
				dataSource: {
					serverPaging: true,
					 schema: {
						 ...
						 $("#KDCQuyLuongCount").text(""+response.total);
					...
					transport: {
						read: {
							...
							url: Constant.BASE_SERVICE_URL + "tblKdcQuyLuongServiceRest/doSearchQuyLuong",
							contentType: "application/json; charset=utf-8",
							type: "POST"
						},					
						parameterMap: function (options, type) {
						
// vm.KDCQuyLuongSearch.keySearch = "01/2018";
						vm.KDCQuyLuongSearch.page = options.page;
						vm.KDCQuyLuongSearch.pageSize = options.pageSize;
						vm.oldSearch=angular.copy(vm.KDCQuyLuongSearch);
						return JSON.stringify(vm.KDCQuyLuongSearch)
						}
					},					 
					pageSize: 10
				} ,
				...
				{
					title : "Thao tác",
					template : '<div class="text-center #=tblKdcQuyLuongId#""> '
							+

							'		<button style=" border: none; " class="#=tblKdcQuyLuongId# icon_table"   uib-tooltip="Cập nhật" translate>'
							+ '			<i class="fa fa-pencil"    aria-hidden="true"></i> '
							+ '		</button>'
							+

							'		<button style=" border: none; " class="#=tblKdcQuyLuongId# icon_table"  uib-tooltip="Hủy bỏ" translate>'
							+
		...
		function doSearch() {
			if($('#KDCQuyLuongAuto').val()===""||$('#KDCQuyLuongAuto').val()==null){
				 vm.KDCQuyLuongSearch.donVi=null;
				 vm.KDCQuyLuongSearch.tenDanhMuc=null;
			}
			vm.showDetail = false;
			var grid = vm.KDCQuyLuongGrid;
		...
		vm.Import = function Import() {
			...
			var title = "Import KDC quỹ lương";
			var windowId = "KDC_QUY_LUONG";
        ...
        function submit(){
			     ...
			     return   $.ajax({
			            url: RestEndpoint.BASE_SERVICE_URL + RestEndpoint.TBL_KDC_QUY_LUONG +"/importQuyLuong?folder="+ vm.folder,
			            type: "POST",
			            data: formData,
        ...
        vm.exportExcelGrid= function(){
			...
			KDCQuyLuongService.exportExcelGrid(vm.oldSearch).then(function(result){
				if (result.fileName) {
        ...
        vm.showHideColumn = function(column) {
			if (angular.isUndefined(column.hidden)) {
				vm.KDCQuyLuongGrid.hideColumn(column);
			} else if (column.hidden) {
				vm.KDCQuyLuongGrid.showColumn(column);
			} else {
				vm.KDCQuyLuongGrid.hideColumn(column);
			}
		...
		function onSave(data){
			vm.KDCQuyLuongSearch.donVi=data.maDanhMuc;
			vm.KDCQuyLuongSearch.tenDanhMuc=data.tenDanhMuc;
			$('#KDCQuyLuongAuto').focus();
		}
	 ...
	 vm.patternOptions={
                ...
                select: function(e) {
                    ...
                    vm.KDCQuyLuongSearch.donVi=dataItem.maDanhMuc;
                    vm.KDCQuyLuongSearch.tenDanhMuc=dataItem.tenDanhMuc;
                ...
                dataSource: {
                    ...
                    transport: {
                        read: function(options) {
							...
							// vị
                            return Restangular.all("tblDanhMucServiceRest/getDeptForAutocomplete").post({pageSize:10, page:1, name:$("#KDCQuyLuongAuto").val().trim()}).then(function(response){
                ...
                change: function(e) {
                	if(processSearch('KDCQuyLuongAuto',vm.selectedPrpject)){
    					 $('#KDCQuyLuongAuto').val("");
    					 vm.KDCQuyLuongSearch.donVi="";
    					 vm.KDCQuyLuongSearch.maDanhMuc="";
