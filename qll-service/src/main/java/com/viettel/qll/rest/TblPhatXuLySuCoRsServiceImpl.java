...
import com.viettel.qll.business.TblPhatXuLySuCoBusinessImpl;
...
public class TblPhatXuLySuCoRsServiceImpl implements TblPhatXuLySuCoRsService {
	@Autowired
	...
	protected final Logger log = Logger.getLogger(TblPhatXuLySuCoRsService.class);
	@Autowired
	TblPhatXuLySuCoBusinessImpl tblPhatXuLySuCoBusinessImpl;

	@Override
	public Response doSearchPhatKPIXLSC(TblPhatXuLySuCoDTO obj) {
		DataListDTO data = tblPhatXuLySuCoBusinessImpl.doSearchPhatKPIXLSC(obj);
	...
	public Response importFile(Attachment attachments, HttpServletRequest request) throws Exception {
		// TODO Auto-generated method stub
				...
				try (InputStream inputStream = dataHandler.getInputStream();) {
					...
					try {
						String result = "";
						result = tblPhatXuLySuCoBusinessImpl.importFile(folderUpload + filePath,request);
						if(result.isEmpty()){
	...
	@Override
	public Response exportExcelGrid(TblPhatXuLySuCoDTO obj) throws Exception {
		// TODO Auto-generated method stub
				try {
					String result = tblPhatXuLySuCoBusinessImpl.exportExcelGrid(obj);
					if (result.isEmpty()) {
						return Response.ok().build();
					} else {
						return Response.ok().entity(Collections.singletonMap("fileName", result)).build();
	...
	public Response downloadImportTemplate() throws Exception {
		try {
			String result = tblPhatXuLySuCoBusinessImpl.downloadImportTemplate();
			if (result.isEmpty()) {
				return Response.ok().build();
			} else {
				return Response.ok().entity(Collections.singletonMap("fileName", result)).build();
