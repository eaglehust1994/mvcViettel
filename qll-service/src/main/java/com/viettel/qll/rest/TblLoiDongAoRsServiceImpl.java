...
import com.viettel.qll.business.TblLoiDongAoBusinessImpl;
...
public class TblLoiDongAoRsServiceImpl implements TblLoiDongAoRsService {

	protected final Logger log = Logger.getLogger(TblLoiDongAoRsService.class);
	...
	@Autowired
	TblLoiDongAoBusinessImpl tblLoiDongAoBusinessImpl;
	@Override
	public Response doSearch(TblLoiDongAoDTO obj) {
		// TODO Auto-generated method stub
		DataListDTO data = tblLoiDongAoBusinessImpl.doSearch(obj);
	...
	public Response importFile(Attachment attachments, HttpServletRequest request) throws Exception {
		...
		try (InputStream inputStream = dataHandler.getInputStream();) {
			...
			try {
				String result = "";
				result = tblLoiDongAoBusinessImpl.importFile(folderUpload + filePath,request);
				if(result.isEmpty()){
	...
	public Response exportExcelGrid(TblLoiDongAoDTO obj) throws Exception {
		// TODO Auto-generated method stub
		try {
			String result = tblLoiDongAoBusinessImpl.exportExcelGrid(obj);
			if (result.isEmpty()) {
				return Response.ok().build();
			} else {
				return Response.ok().entity(Collections.singletonMap("fileName", result)).build();
	...
	public Response downloadImportTemplate() throws Exception {
		try {
			String result = tblLoiDongAoBusinessImpl.downloadImportTemplate();
			if (result.isEmpty()) {
				return Response.ok().build();
			} else {
				return Response.ok().entity(Collections.singletonMap("fileName", result)).build();
