...
import com.viettel.qll.business.TblDmKiCaNhanBusinessImpl;
...
public class TblDmKiCaNhanRsServiceImpl implements TblDmKiCaNhanRsService {

	protected final Logger log = Logger.getLogger(TblDmKiCaNhanRsService.class);
	@Autowired
	TblDmKiCaNhanBusinessImpl tblDmKiCaNhanBusinessImpl;
	...
	public Response doSearchKiCaNhan(TblDmKiCaNhanDTO obj) throws Exception {
		...
		DataListDTO data = tblDmKiCaNhanBusinessImpl.doSearchKiCaNhan(obj, request);
		if (data == null) {
	...
	@Override
	public Response importKiCaNhan(Attachment attachments, HttpServletRequest request) throws Exception {
		...
		try {
			...
			try {
				String result = tblDmKiCaNhanBusinessImpl.importKiCaNhan(folderUpload + filePath,request);
				if (result.isEmpty()) {
	...
	public Response exportExcelGrid(TblDmKiCaNhanDTO obj) throws Exception {
		...
		try {
			String result = tblDmKiCaNhanBusinessImpl.exportKICaNhan(obj);
			if (result.isEmpty()) {
				return Response.ok().build();
			} else {
				return Response.ok().entity(Collections.singletonMap("fileName", result)).build();
