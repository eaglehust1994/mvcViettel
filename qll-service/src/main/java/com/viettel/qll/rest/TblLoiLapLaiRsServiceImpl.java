...
import com.viettel.qll.business.TblLoiLapLaiBusinessImpl;
...
public class TblLoiLapLaiRsServiceImpl implements TblLoiLapLaiRsService {

	protected final Logger log = Logger.getLogger(TblLoiLapLaiRsService.class);
	@Autowired
	TblLoiLapLaiBusinessImpl tblLoiLapLaiBusinessImpl;
	...
	@Override
	public Response doSearch(TblLoiLapLaiDTO obj) {
		// TODO Auto-generated method stub
		DataListDTO data = tblLoiLapLaiBusinessImpl.doSearch(obj);
	...
	public Response importFile(Attachment attachments, HttpServletRequest request) throws Exception {
		...
		try (InputStream inputStream = dataHandler.getInputStream();) {
			...
			try {
				String result = "";
				result = tblLoiLapLaiBusinessImpl.importFile(folderUpload + filePath,request);
				if(result.isEmpty()){
	...
	@Override
	public Response exportExcelGrid(TblLoiLapLaiDTO obj) throws Exception {
		// TODO Auto-generated method stub
				try {
					String result = tblLoiLapLaiBusinessImpl.exportExcelGrid(obj);
					if (result.isEmpty()) {
						return Response.ok().build();
					} else {
						return Response.ok().entity(Collections.singletonMap("fileName", result)).build();
	...
	public Response downloadImportTemplate() throws Exception {
		try {
			String result = tblLoiLapLaiBusinessImpl.downloadImportTemplate();
			if (result.isEmpty()) {
				return Response.ok().build();
			} else {
				return Response.ok().entity(Collections.singletonMap("fileName", result)).build();
