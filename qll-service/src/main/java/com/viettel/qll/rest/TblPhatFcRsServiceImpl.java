...
import com.viettel.qll.business.TblPhatFcBusinessImpl;
...
public class TblPhatFcRsServiceImpl implements TblPhatFcRsService {

	protected final Logger log = Logger.getLogger(TblPhatFcRsService.class);
	@Autowired
	TblPhatFcBusinessImpl tblPhatFcBusinessImpl;
	...
	public Response doSearchPhatFc(TblPhatFcDTO obj) throws Exception {
		DataListDTO data = tblPhatFcBusinessImpl.doSearch(obj,request);
        if (data == null) {
	...
	public Response importFile(Attachment attachments, HttpServletRequest request) throws Exception {
		...
		try {
			...
			try {
				String result = tblPhatFcBusinessImpl.importFile(folderUpload + filePath,request);
				if(result.isEmpty()){
	...
	public Response exportExcelGrid(TblPhatFcDTO obj) throws Exception {
		try {
			String result = tblPhatFcBusinessImpl.exportExcelGrid(obj);
			if (result.isEmpty()) {
				return Response.ok().build();
			} else {
				return Response.ok().entity(Collections.singletonMap("fileName", result)).build();
	...
	public Response downloadImportTemplate() throws Exception {
		// TODO Auto-generated method stub
		try {
			String result = tblPhatFcBusinessImpl.downloadImportTemplate();
			if (result.isEmpty()) {
				return Response.ok().build();
			} else {
				return Response.ok().entity(Collections.singletonMap("fileName", result)).build();
