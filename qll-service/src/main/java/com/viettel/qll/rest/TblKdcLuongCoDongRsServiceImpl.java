...
public class TblKdcLuongCoDongRsServiceImpl implements TblKdcLuongCoDongRsService {

	protected final Logger log = Logger.getLogger(TblKdcLuongCoDongRsService.class);
	...
	@Autowired
	TblKdcLuongCoDongBusinessImpl tblKdcLuongCoDongBusinessImpl;
	@Context
	HttpServletRequest request;

	@Override
	public Response doSearchKDCLuongCD(TblKdcLuongCoDongDTO obj) throws Exception {
		DataListDTO data = tblKdcLuongCoDongBusinessImpl.doSearchKDCLuongCD(obj, request);
		if (data == null) {
	...
	@Override
	public Response importKDCLuongCD(Attachment attachments, HttpServletRequest request) throws Exception {
		...
		try {
			...
			try {
				String result = tblKdcLuongCoDongBusinessImpl.importKDCLuongCD(folderUpload + filePath,request);
				if (result.isEmpty()) {
	...
	public Response exportExcelGrid(TblKdcLuongCoDongDTO obj) throws Exception {
		...
		try {
			String result = tblKdcLuongCoDongBusinessImpl.exportKDCLuongCD(obj);
			if (result.isEmpty()) {
				return Response.ok().build();
			} else {
				return Response.ok().entity(Collections.singletonMap("fileName", result)).build();
